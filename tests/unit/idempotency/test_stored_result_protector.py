...
def test_protector_round_trips_nested_json_results():
    protector = AESGCMStoredResultProtector(
        active_key_id="v1", keys={"v1": os.urandom(32)}
    )
    payload = {
        "value": {"version": 2, "enabled": True, "avatar": None, "tags": ["a", "b"]}
    }
    assert (
        protector.restore(protector.protect(payload, aad=b"scope"), aad=b"scope")
        == payload
    )
