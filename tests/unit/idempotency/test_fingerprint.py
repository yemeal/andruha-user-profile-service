...
def test_decimal_fingerprint_does_not_round_to_current_context_precision():
    from decimal import localcontext

    left = {"amount": Decimal("123456789.1234567891")}
    right = {"amount": Decimal("123456789.1234567892")}
    with localcontext() as context:
        context.prec = 6
        low_precision = compute_request_fingerprint(left)
        assert low_precision != compute_request_fingerprint(right)
    assert low_precision == compute_request_fingerprint(left)
