...
if format_version ~= '2' or not current_fingerprint then
    return {5}
end
...
if format_version == '2' and state == 'processing'
...
if format_version == '2' and state == 'processing'
    ...
    local ttl_ms = math.min(tonumber(ARGV[3]), tonumber(ARGV[5]) - now_ms)
    if ttl_ms <= 0 then
...
local state = redis.call('HGET', KEYS[1], 'state')
local lease_owner = redis.call('HGET', KEYS[1], 'lease_token')
if format_version == '2' and state == 'processing' and lease_owner == ARGV[1] then
...
"""
    ...
    def __init__(
        ...
        redis_client: Redis,
        *,
        key_namespace: str = "andruha-user-profile-service:idempotency:v2",
    ) -> None:
        ...
        *,
        ...
        try:
            completed = await self._redis.eval(
                ...
                str(lease_token),
                encoded,
                str(cache_ttl_seconds * 1000),
                result.request_fingerprint.hex(),
                str(int(result.expires_at.timestamp() * 1000)),
            )
