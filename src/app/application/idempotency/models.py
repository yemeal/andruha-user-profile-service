...
class StoredResult(BaseModel):
    ...
    result_type: str = Field(
        min_length=1,
        max_length=100,
        description="Формат результата; completion зарезервирован для отметки без payload",
    )
    ...
    @classmethod
    def completion(cls) -> StoredResult:
        """Успешное выполнение без сохранения результата handler."""
        return cls(result_type="completion")

    @property
    def is_completion_only(self) -> bool:
        return self.result_type == "completion"
    ...
    def validate_replayability(self) -> StoredResult:
        if self.is_completion_only:
            if any(
                ...
                raise ValueError(
                    "Completion marker must not carry a result or resource"
                )
            return self
...
class HotDegradationStage(StrEnum):
    """Bounded metric labels for failures in the volatile layer."""

    BEGIN = "begin"
    LEASE = "lease"
    COMPLETE = "complete"
    RELEASE = "release"
    LEASE_LOST_ON_COMPLETE = "lease_lost_on_complete"
    LEASE_LOST_ON_RELEASE = "lease_lost_on_release"
